 ...
 */
const INDEX = 'app/pages/settings/index.vue'

describe('the settings overview', () => {
  const source = readFileSync(INDEX, 'utf8')
  const targets = [...source.matchAll(/to: '\/settings\/([a-z-]+)'/g)].map((match) => match[1])
  ...
  it.each(targets)('/settings/%s has a page behind it', (name) => {
    expect(existsSync(`app/pages/settings/${name}.vue`)).toBe(true)
  })
})
