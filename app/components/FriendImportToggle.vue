...
<template>
  <section v-if="prefs" class="flex flex-col gap-3">
    ...
    <p class="text-fid-sm text-fid-text-muted">{{ st.search.dealers.ordersAlways }}</p>
    ...
    -->
    <WhyNote :label="st.search.dealers.whyLabel">
      {{ st.search.dealers.why }}
      <span class="font-fid-mono">/users/…/friends</span>
      {{ st.search.dealers.whyAfter }}
    </WhyNote>
  </section>
</template>
