...
| M21.3 The build · image and job done 2026-09-12, first full run failed at 90 s on a label without an id (fixed), second run pending | `fetch.ts` (the dump from data.discogs.com, hashed against CHECKSUM.txt), `run.ts` (fetch, build, check, atomic `current` swap, two builds kept, dump files deleted as loaded, daily look for a new month, `status.json`), `deploy/catalogue.Dockerfile` in the Images workflow, the service in `compose.homelab.yml`; 26 tests. The full run on home-deb is timed and sized as soon as the image is published | a weekend with the home lab |
