...
**Started 2026-09-21** with #1 and #2, which are one observation twice: a list of finds in
which twenty cards say the same sentence, standing in the order the scan happened to write
them. #5 turned out to be half built already — checked in the code, not read off this
column, which is the mistake this file keeps making. #3 and #4 are open.
...
| 2 | **A tie-break that says its name.** Twenty finds at 48: "by score" orders nothing among them, and the list is whatever the scan happened to write first. A second key when scores tie — the price, or the year — and the sort plate saying so: "Score, dann Preis" | The same digs | hours | **Done 2026-09-21.** The price, not the year: two records you have the same reason to want are told apart by what they cost, which is the question the next click asks anyway. `tieBreak` sits outside the direction, like the score does under every other key — turning "by score" round asks for the weakest find first, not the dearest one — and an expired price goes last from either end. The tab in force reads "SCORE ↓ · DANN PREIS" |
