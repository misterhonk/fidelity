...
**M0 through M20 are done, and so are M26 through M32, M34 and M36.** What is open is M33
(two of its five rows shipped on 2026-09-21), M35's last reading, and the entries below
that wait on a signature or on money.

**Three headings were wrong on 2026-09-21 and are now right.** M34 stood at "in progress"
with all six of its phases ticked since 20 September; M35.4 stood open at "a day" with
every word of it in the packs; M33 #5 stood as a candidate with half of it built. All
three were found the way the warning below says to find them — by opening the code for the
row rather than reading the Note column — and none of them was a milestone anybody had
forgotten to do. They were milestones nobody had come back to tick.
...
## M33 · The third walk with the demo account → in progress
...
everything else a plate — and against what the device actually held. Two things were found
and fixed on the spot (`v0.91.1`).

**Done 2026-09-21, four of five rows.** #1 and #2 first, then #3 and #4 the same day. Two
of the four turned out to be partly built or partly wrong once the code was opened rather
than the Note column — #2's premise (the find list already ordered on the price) and #5,
which is half there. That is the mistake this file keeps making, and it is cheaper to find
it before the work than after.

| # | Candidate | Serves | Cost | Note |
|---|---|---|---|---|
| 1 | **The reason, once.** Twenty of twenty-seven cards said the same sentence: "*X* steht schon in deiner Sammlung – diese Platte nicht." A reason that repeats is not a reason, it is a category. Where the lead signal is the same across a stretch of finds, the card carries it as a plate — "IM REGAL · Four Tet · 5" — and the sentence stays for the cards that have more to say | Every dig against a shop that stocks what you already collect — most of them | hours | **Done 2026-09-21.** `repeatedLeads` in `app/utils/digview.ts` names the leads that lead at least five finds *and* a quarter of the list — five because four of anything is still four things you read one at a time, a quarter rather than a half so two signals dividing a list between them can both be one. `MatchList` computes it over the list as it stands and provides it (`useLeads`); the card and the compact row draw the plate and hand `reasonFor` the lead to leave out, so a card with a second signal says the second thing and one with nothing left says nothing. Outside a long list — the top five, the stack, a shared dig — nobody provides anything and every sentence stands. The plate has **its own words** (`plate` in `app/i18n/reason.ts`), not the filter chips': "ARTIST · PROBE 3 · 5" over a card headed "Probe 3" says the name twice and the point not at all, which is what the first attempt looked like before it was drawn. So "IM REGAL · 5", and a name only where the card carries it nowhere else — a producer, a style, a format |
| 2 | **A tie-break that says its name.** Twenty finds at 48: "by score" orders nothing among them, and the list is whatever the scan happened to write first. A second key when scores tie — the price, or the year — and the sort plate saying so: "Score, dann Preis" | The same digs | hours | **Done 2026-09-21, and half of it was a misreading.** The find list already ordered on the price: `bestPerRelease` has ranked that way since M2 and `Array.sort` is stable, so the view inherited it through a comparison that read the score alone. The premise of this row was wrong about that screen — what was missing was that **nothing said so**, and the tab in force reads "SCORE ↓ · DANN PREIS" now. The row was right about a screen it did not name: the start rail read the `by-dig-score` index and reversed it, which among equal scores gave the dearest first (measured on Martin's own start screen the same day: five finds at 48 at €16, €16, €15, €14.50, €11). The rule is one function in `shared/score.ts` now — `byScoreThenPrice`, read by the worker's ranking, the rail and the view — with a test holding them together, because a screen that is right by accident is one refactor from being wrong in silence |
| 3 | **One head, one loud thing.** Above the finds stand "fatplastics nochmal graben", "Rest bauen", "Eine nach der anderen" and a bordered box with "Preise auffrischen (32)" — four calls before the first record. The refresh becomes a plate action in the head's line, the stack mode a plate, the horizon's rest a sentence in the settings | Every dig older than six hours | half a day | **Done 2026-09-21.** One `<nav>` of plates under the head, in the order somebody asks for them: look again, look differently, repair the prices, pass it on. No fill, no frame, no box round any of them — the stack lost its accent border and the refresh lost the box it sat in. What that box also held stays as a sentence, because it is one: the six hours and what a refresh costs, now read as "Auffrischen kostet 17 Abfragen, also rund eine Minute" rather than a bare figure under its own button. The horizon's rest was already a link inside the basis sentence and stays one |
| 4 | **"Randnotiz ?" once, not per card.** The help mark under every score repeats twenty-seven times; one "Warum?" at the head of the list, the way the plan box does it | Sixty-word rule, in spirit | an hour | **Done 2026-09-21.** A `WhyNote` at the head of the list, and `ScoreMark` stands its own mark down wherever that head exists. Read off the same fact the plate is (`useLeads().inList`): a provider exists exactly where `MatchList` draws, so the mark stays on every score that stands alone — the sheet, the top five, the stack, the basket. Not in the crate, whose tiles carry no score to explain |
| 5 | **Covers for what is on the screen first.** A find's cover costs one `/releases/{id}` at the pace of one per 1.2 s; twenty-seven finds are half a minute of dark tiles, and the crate — sleeves only — is a dark wall until then. The visible rows first (an `IntersectionObserver` on the list), nothing while the tab is hidden, and the hub's cover pool where a hub is connected | The crate, and every list on a slow evening | half a day | **Half built, checked in the code 2026-09-21, not from this column.** `app/composables/useCovers.ts` has the observer, one map for the whole app, one fetch in flight and a budget of 120 a visit. What is not there is the other two: nothing asks whether the tab is hidden, and no hub pool is consulted |
...
| 36.5 The comparison | Two full visits side by side: new, gone, still there | **Done 2026-09-21**, `v0.109.0`: `dig.compare` sets this full dig against the full dig before it (`compareDigs`): what this one found that the earlier had not, what the earlier found that is marked gone, and the rest of the earlier finds; a folded section under the top five with three compact lists, and "gone unknown" where this dig did not see the whole shop |
...
| Phase | Delivers | State |
|---|---|---|
| M34.2 The measurement | One `GET /marketplace/listings/{id}` with the token, its verdict as a plate in the account ("Discogs names postage to you: yes/no"); `min_order_total` and `payment` into the inventory schema and onto the dealer; the currency check in `summarise`; Discogs' default weights (230 g LP, 135 g 10", 85 g CD) as constants | **Measured 2026-09-16** on Martin's device: **yes.** With a token `GET /marketplace/listings/{id}` carries `shipping_price` (postage for that one listing to the account's address, converted) and `original_shipping_price` (exact, seller currency); `shipping_is_blocked` becomes `null`; `min_order_total` and `payment` are there. Recorded in docs/02 §5. The plate in the account is moot — the app reads the figure instead. **Done 2026-09-16**, `v0.93.0`: paste and basket refresh keep it per line (`ListingPostage`), a basket of one totals with it, beside a table it is the floor, the profile's postage plate leads with it; `min_order_total` and `payment` off the rows onto the dealer; the default weights as constants. The tier text for two or more records stays 34.3's business |
| M34.5 The seven fixes | Hub: the `:dealer` path parameter validated, `seenAt` in the future refused, the key limiter on the secret door too, avatar URLs held to `https://i.discogs.com`; client: bounds on the shops schema, the same avatar check at the Discogs boundary, `error.message` redacted in the worker's envelope; a Content-Security-Policy shipped by `.htaccess` and nginx with `connect-src https:` (the hub origin is the user's), `frame-src` for the two YouTube hosts, `style-src 'unsafe-inline'` for the bound styles; `docs/08` brought to what ships | **Done 2026-09-16**, `v0.91.2`/`v0.91.3` — thirteen screens walked behind the policy without a violation; the app image needed a second release because `.dockerignore` hid the new `nginx.conf`. **2026-09-20:** the policy had allowed the audio preview's frame and not its script, and every play button was gone from `v0.91.2` on; `script-src` names `www.youtube.com` now, with a test. The same day the consent moved to where the clips are (`AudioConsent.vue`, ADR-012 amended) and the switch from Your data to Search → Hear a record |
| M34.1 Masthead and plates | Profile head with the trust line, count before percentage, negatives spelled out, `location` never shown whole; one filled action, actions with icon *and* word; sections as plates in the order of the buyer's questions with bullet bars and a median tick; "Why?" folds instead of sentences; aria names; the list with plate words as reasons, country codes, "+n new", the verdict pair per row, sort as a word, filters as chips, "load more", return to the same place, a placeholder on the right; swipe from shop to shop in the sheet; five starter shops for an empty device; the foot as one field; discovery from orders removed | **Done 2026-09-16**, `v0.92.0` — trust line, one fill, plates in the buyer's order with folds, undo line, plate-word reasons, country codes, plate tabs for the order, chip counts, j/k and the sheet's arrows, the placeholder on the right, discovery moved to the settings and the order import writing its seller. Left out on purpose: bullet bars and the median tick (the label bars stay), the verdict pair per row, "return to the same place", the starter shops — they go with 34.4 when the page is split, or fall away |
| M34.4 One call, three indexes | `dealer.overview`; indexes on `dealers`, `feedback`, `collection`; hub contributions after the read, concurrent and abortable; avatar pending set; the page split into four components; language packs lazy | **Done 2026-09-16**, `v0.94.0` — `dealer.overview`, schema 15 with `by-dealer` and the multi-entry `by-label` (no index on `dealers`: a few dozen rows), the hub asked after the draw and cancelled with the screen, a profile asked once per session, the page as orchestrator over `ShopRound`, `ShopList`, `ShopProfile`, `ShopFoot` with `useShopList`; and the 34.1 leftovers: bullet bars with the median tick, the verdict pair per row, five verified starter shops. The round shrank to one line with a fold (reported as taking the whole screen). Left out: lazy language packs, the pack rides in the page's own chunk |
| M34.3 Shipping profile v2 | The model (units, format, method, free-over, weight by Discogs' defaults); migration; the read-off field as the main road, the field next to "bought", the pasted cart text, the postage of one's own order (after an ADR); the parser as a proposal to confirm, with the flat-rate rule and the pause rule; the catalogue serving `qty`; the "2x" rule in the app; the Shipping section on the profile; postage on the row as an estimate; basket, comparison and plan lines on it; the strategy switch; "shipping changes when you deselect"; hub routes for profiles with "confirmed by n"; the seller text expiring with its dig | **First helping 2026-09-17**, `v0.100.0`: the parser reads the measured British text in full (ordinals, two unit words, a dash before the price, "upto", a heading with an aside, an article or two places, the step written price-first, asides only kept when they are a price) and the free-over threshold per destination; a table in the seller's currency is converted at Discogs' own rate off a fresh line and refused without one; free postage above the threshold with the distance to it on the card; the read-off field on the basket, cutting the user's tier around the count. Postage on the shops row in `v0.101.0` (Discogs' figure, the user's table, or a reading of the text, in that order, unconverted). **The pasted cart page in `v0.102.0`** (2026-09-17, against three of Martin's carts and seven shops): the paste box reads a copied cart page by its labels, in either interface language, and notes per shop Discogs' postage for that many records as a user tier that runs as far as "add up to n more at no additional cost" says, the free-postage threshold (kept on the dealer, ahead of the parser's reading), and the minimum order from the shortfall line; no request, no listing id, so no basket line comes out of it. Fixtures `tests/fixtures/cart-text.ts`. **The model with units in `v0.103.0`** (2026-09-17, against the seller texts of green_hell, recordsale-de and wheniamfortyfive in `shipping-notes.ts`): a tier carries the unit its count counts (`ShippingTier.unit`: records, singles, CDs; absent means records), the parser reads "1-10 7inches 4,50" and "1-10 cd 4,50" as tables of their own beside "1-20 LPs 8,00", a base price with a step per unit ("Base Price: €2,90 + €2,00 per LP/12inch"), a ceiling per destination ("3,90 EUR -> Germany", the ceiling alone is the rate where the block gives none, which the cart confirmed for one and two records), one rate for any number ("unlimited number of 45s for €6,-"), and the decimals of a price are no longer a count; a basket line keeps Discogs' format string (`BasketItem.format`), `unitsOf` reads unit and count off it, `summarise` picks the singles table for a parcel of singles and the records table for a mix, and a double LP counts as two only where the shop says so (`doubleCounts`). Weights stay a label (`byWeight`): no honest grams per record. **The own-order postage in `v0.104.0`** (2026-09-17, [ADR-017](adr/017-postage-from-your-own-order.md)): the order import reads `shipping` and keeps it as the shop's tier for that many records, source `order`, ranked with the user's own table everywhere, labelled "from your order"; the schema still names no price, grade or address. **The catalogue `qty` in `v0.105.0`** (2026-09-17): `release/{id}` and every family sibling answer `discs`, summed like the sync's count; typed optional on the client, nothing reads it yet — the basket counts off Discogs' own format string on the line, and the family's count is there for M34.6's fill-up candidates. **The hub route with "confirmed by n" in `v0.106.0`** (2026-09-20): the hub keeps one vote per access key and shop, hands out the ladder most keys agree on (a tie goes to the newer), and answers `confirmedBy`; the client carries it to the basket's shipping line ("shared, 3 people typed the same"). Behind the secret door or on an open hub every contributor is the same owner, so the count is one there — the hub cannot tell two people apart without keys, and says so rather than guessing. **M34.3 done.** |
| M34.6 Filling the parcel, the tempo | "For the same postage there is room for …" from the shop's last dig up to the tier's edge; "n % of the finds were gone by the next dig" from the round | **Done 2026-09-20**, `v0.107.0`: the basket card says how much more fits at this postage (`BasketSummary.room`, the edge of the tier the parcel is in, "add as many as you like" where it has none) and how many the last dig here offers to fill it; the shops row says how far its one-record figure carries ("from €4.50 postage, up to 3 records"); the profile's movement plate carries the tempo off the last two full digs (`worker/dealers/tempo.ts`: of the finds from one, how many the next no longer saw, and the days between) — full digs only, since a new-arrivals dig never sees what left; sold and taken down are not told apart, and the sentence does not claim to |

## M35 · The voice → in progress
...
| M35.2 The first screens | Welcome, Start, Dig, the reasons on a find, Shops — both languages, every sentence read aloud; first because everybody sees them and the reasons are the idea itself | **Done 2026-09-17**: Welcome and Shops in `v0.95.0` ("put away / weglegen", "die Runde", "Angebote", "Your shops" for the list); Start, the reasons on a find and the Dig in `v0.96.0` ("Funde" for "Treffer", "Digging through" for "Scanning", "dug" for "scanned", the deep dig, no dash chains; 93 browser checks moved with the words) |
| M35.3 The rest | Basket, Collection, Wantlist, Places, Settings, errors — both languages | **Done 2026-09-17**: Basket and Collection in `v0.97.0`; the common pack in `v0.98.0`; Settings with its help chapters and the pressing notes in `v0.99.0`. Ten packs under the relapse test. Left as it is: `compared.ts`, the comparison page, which names Fidelity beside other products and needs the third person |
| M35.4 The words that are buttons | hide → put away / weglegen, the round → die Runde, scan → dig, listings → Angebote in sentences; "watch / beobachten" stays (Martin, 2026-09-17); the 181 text assertions in 63 specs move in the same change | **Done**, and this row said "a day" until 2026-09-21 with every word of it shipped — checked in the packs, not read off this column: `weglegen` and "put away" in `dealers.ts`, `die Runde` in `de.ts` and `dealers.ts`, `Angebote` for listings in a sentence in `de.ts`, and "scan" gone from every pack the relapse test watches. It rode along with M35.2 and M35.3 rather than as a change of its own, which is why nobody came back to tick it |
| M35.5 The proof | Martin reads both languages against the demo collection and marks what still sounds like an office; a second short pass | **First reading done 2026-09-21**, `v0.109.1`, on Martin's word: every screen and the find's sheet dumped from the seeded build in both languages (21 routes) and read as a whole. Found and fixed: "1 Wünsche" and "1 Platten gesucht" (plurals on the start, the wantlist, the settings and the welcome), Discogs' own "€610.00" beside "1.220,00 €" on the map's value line, "Horizont/Horizon" as a heading, a button and a help question (now "your artists and labels"), "Merkliste" beside the tab called "Gemerkt", and the wantlist's lead cut into a sentence. The rest read as the counter; Martin's own pass stays welcome |
